...
export const regFace = async (req, res) => {
  ...
  if (!embeddings || !Array.isArray(embeddings) || embeddings.length !== 128) {
...
export const verifyFace = async (req, res) => {
  ...
  const possibleFaces = faces.filter((face) => {
    const embeddingsMatch = embeddingsCheck(embeddings, face.embeddings, 0.35);
    if (embeddingsMatch) {
      return face;
    }
  });
  ...
  const date = new Date();
  const formatted = date.toLocaleDateString("en-CA"); //2026-02-15
